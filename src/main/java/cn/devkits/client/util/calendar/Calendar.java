...
public final class Calendar implements Serializable, Comparable<Calendar> {
    ...
    /**
     * 农历字符串，没有特别大的意义，用来做简单的农历或者节日标记 建议通过lunarCakendar获取完整的农历日期
     */
    ...
    /**
     * 计划，可以用来标记当天是否有任务,这里是默认的，如果使用多标记，请使用下面API using addScheme(int schemeColor,String scheme); multi
     * scheme
     */
    private String scheme;

    /**
     * 各种自定义标记颜色、没有则选择默认颜色，如果使用多标记，请使用下面API using addScheme(int schemeColor,String scheme); multi scheme
     */
    private int schemeColor;


    /**
     * 多标记 multi scheme,using addScheme();
     */
    ...
    final void mergeScheme(Calendar calendar, String defaultScheme) {
        if (calendar == null)
            return;
        setScheme(StringUtils.isEmpty(calendar.getScheme()) ? defaultScheme : calendar.getScheme());
